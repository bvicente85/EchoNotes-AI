...
async function transcribeFileWithModels(filename: string) {
  ...
  // Try different models to bypass daily free tier quota limits
  ...
  console.error(`All models failed to transcribe ${filename}.`);
  return false;
}

async function run() {
  console.log("Starting transcription of remaining files with model fallback...");
  ...
  for (const file of remainingFiles) {
    const success = await transcribeFileWithModels(file);
    if (success) {
      console.log(`Successfully processed ${file}.`);
    } else {
      console.log(`Failed to process ${file} with all fallback models.`);
    }
  }
  console.log("\nProcess completed!");
}
