...
async function transcribeFileWithGroq(filename: string, maxAttempts = 5) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      console.log(`Attempt ${attempt}/${maxAttempts}: Transcribing directly with Groq (Whisper + Llama)...`);
      ...
      const report = await generateMeetingReport(
        ...
        'groq-llama-3.3' // modelOverride forces Groq
      );
      ...
      if (attempt < maxAttempts) {
        const waitSec = 65;
        console.log(`Groq rate limit. Sleeping for ${waitSec} seconds before retrying...`);
        await sleep(waitSec * 1000);
      } else {
        console.error(`All ${maxAttempts} attempts failed for ${filename}.`);
      }
    }
  }
  return false;
...
async function run() {
  // Sleep 45 seconds before starting to let any pending rate limits clear
  console.log("Sleeping 45 seconds to let the Groq hourly rate limit clear...");
  await sleep(45000);
  
  console.log("Starting direct Groq transcription process...");
  ...
  for (let i = 0; i < remainingFiles.length; i++) {
    ...
    const success = await transcribeFileWithGroq(file);
    
    if (success && i < remainingFiles.length - 1) {
      const waitBetween = 65;
      console.log(`\nSleeping for ${waitBetween} seconds to let the rate limit reset before the next file...`);
      await sleep(waitBetween * 1000);
    }
  }
  console.log("\nDirect Groq batch transcription process completed!");
}
