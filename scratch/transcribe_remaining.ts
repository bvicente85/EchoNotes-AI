...
async function transcribeFileWithRetry(filename: string, maxAttempts = 5) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      console.log(`Attempt ${attempt}/${maxAttempts}: Sending to Gemini API (Portuguese)...`);
      ...
      if (attempt < maxAttempts) {
        const waitSec = 65;
        console.log(`Rate limit or API error. Sleeping for ${waitSec} seconds before retrying...`);
...
async function run() {
  console.log("Starting transcription of remaining files with rate-limiting protection...");
  ...
  for (let i = 0; i < remainingFiles.length; i++) {
    ...
    // Transcribe the file
    const success = await transcribeFileWithRetry(file);
    
    // Sleep between files to prevent hitting the rate limit
  ...
  console.log("\nBatch transcription process completed!");
}
